...
{
    internal class FamilyEventParser : AbstractEventParser<FamilyEvent>
    {
        /// <summary>
        /// Constructor
        /// </summary>
        ...
        public FamilyEventParser(GedcomParser gedcomParser, StringTree stringTree, FamilyEvent loadInto) : base(gedcomParser, stringTree, loadInto)
        {
        }

        public override void Parse()
        {
            _loadInto.Type = EnumExtensions.ParseDescriptionToEnum<FamilyEventType>(_stringTree.Tag);
            ParseYNull();
            if(_stringTree.Children != null)
            {
                foreach(var ch in _stringTree.Children)
                {
                    ...
                    else if (Tag.RESTRICTION.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but restriction notice was specified for family event on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    else if (Tag.RELIGION.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but religious affiliation was specified for family event on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    ...
                    else if (Tag.WEB_ADDRESS.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but WWW URL was specified for family event on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    else if (Tag.FAX.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but fax number was specified for family event on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    else if (Tag.EMAIL.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but email was specified for family event on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    else if (Tag.HUSBAND.Desc().Equals(ch.Tag))
                    {
                        _loadInto.HusbandAge = ParseStringWithCustomFacts(ch.Children[0]);
                    }
                    else if (Tag.WIFE.Desc().Equals(ch.Tag))
                    {
                        _loadInto.WifeAge = ParseStringWithCustomFacts(ch.Children[0]);
                    }
