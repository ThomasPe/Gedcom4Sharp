...
{
    internal class IndividualAttributeParser : AbstractParser<IndividualAttribute>
    {
        /// <summary>
        /// Constructor
        /// </summary>
        ...
        public IndividualAttributeParser(GedcomParser gedcomParser, StringTree stringTree, IndividualAttribute loadInto) : base (gedcomParser, stringTree, loadInto)
        {
        }

        public override void Parse()
        {
            _loadInto.Type = EnumExtensions.ParseDescriptionToEnum<IndividualAttributeType>(_stringTree.Tag);
            if(IndividualAttributeType.FACT.Desc().Equals(_loadInto.Type) && G55())
            {
                _gedcomParser.Warnings.Add($"FACT tag specified on a GEDCOM 5.5 file at line {_stringTree.LineNum}, but FACT was not added until 5.5.1. Data loaded but cannot be re-written unless GEDCOM version changes.");
            }
            ...
            if(_stringTree.Children != null)
            {
                foreach(var ch in _stringTree.Children)
                {
                    ...
                    else if (Tag.WEB_ADDRESS.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but WWW URL was specified for {_loadInto.Type} attribute on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    else if (Tag.FAX.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but fax was specified for {_loadInto.Type} attribute on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    else if (Tag.EMAIL.Desc().Equals(ch.Tag))
                    {
                        ...
                        if (G55())
                        {
                            _gedcomParser.Warnings.Add($"GEDCOM version is 5.5 but email was specified for {_loadInto.Type} attribute on line {ch.LineNum}, which is a GEDCOM 5.5.1 feature. Data loaded but cannot be re-written unless GEDCOM version changes.");
                        }
                    }
                    else if (Tag.ADDRESS.Desc().Equals(ch.Tag))
                    {
                        _loadInto.Address = new Address();
                        new AddressParser(_gedcomParser, _stringTree, _loadInto.Address).Parse();
                    }
